...
IMPLEMENT_DYNAMIC(Cmydia10, CDialogEx)

Cmydia10::Cmydia10(CWnd* pParent /*=NULL*/)
	: CDialogEx(Cmydia10::IDD, pParent)
{
...
void Cmydia10::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(Cmydia10, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_QUERYDRAGICON() 
END_MESSAGE_MAP()
...
BOOL Cmydia10::OnInitDialog()
{
	...
	CreateThread(NULL,0,InitTank3,this,0,NULL);
	CreateThread(NULL,0,MyPaint3,this,0,NULL);//启动线程*

	CreateThread(NULL,0,PlayerButtle3,this,0,NULL);
	CreateThread(NULL,0,PlayerMove3,this,0,NULL);
	CreateThread(NULL,0,EnemyMove03,this,0,NULL);
	CreateThread(NULL,0,EnemyMove13,this,0,NULL);
	CreateThread(NULL,0,EnemyMove23,this,0,NULL);
	CreateThread(NULL,0,EnemyMove33,this,0,NULL);
	CreateThread(NULL,0,EnemyMove43,this,0,NULL);
	CreateThread(NULL,0,EnemyMove53,this,0,NULL);
	CreateThread(NULL,0,EnemyMove63,this,0,NULL);
	CreateThread(NULL,0,EnemyMove73,this,0,NULL);
	

	return TRUE;  // 除非将焦点设置到控件，否则返回 TRUE
}

void Cmydia10::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR Cmydia10::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

DWORD WINAPI MyPaint3(LPVOID lpParam)			//贴图线程
{
	Cmydia10 *c=(Cmydia10*)lpParam;
	...
	c->Back.LoadBitmap(IDB_BITMAP18);//进入关卡的地图
...
DWORD WINAPI InitTank3(LPVOID lpParam)
{
	...
	c->p.x=1100,c->p.y=50;
	c->Enemy[0].InitTank(3,4,12,1,2,1,c->p);
	c->Enemy[0].LoadImage();
	c->p.x-=10,c->p.y+=25;
	c->BEnemy[0].InitBullet(4,1,20,2,c->p);
	c->BEnemy[0].LoadImage();

	c->p.x=900,c->p.y=50;
	c->Enemy[1].InitTank(3,3,12,1,2,2,c->p);
	c->Enemy[1].LoadImage();
	c->p.x-=10,c->p.y+=25;
	c->BEnemy[1].InitBullet(3,1,20,2,c->p);
	c->BEnemy[1].LoadImage();
	
	c->p.x=700,c->p.y=50;
	c->Enemy[2].InitTank(3,2,12,1,2,3,c->p);
	c->Enemy[2].LoadImage();
	c->p.x-=10,c->p.y+=25;
	c->BEnemy[2].InitBullet(2,1,20,2,c->p);
	c->BEnemy[2].LoadImage();

	c->p.x=500,c->p.y=50;
	c->Enemy[3].InitTank(3,3,12,1,2,4,c->p);
	c->Enemy[3].LoadImage();
	c->p.x-=10,c->p.y+=25;
	c->BEnemy[3].InitBullet(3,1,20,2,c->p);
	c->BEnemy[3].LoadImage();

	c->p.x=300,c->p.y=50;
	c->Enemy[4].InitTank(3,4,8,1,2,5,c->p);
	c->Enemy[4].LoadImage();
	c->p.x-=10,c->p.y+=25;
	c->BEnemy[4].InitBullet(4,1,20,2,c->p);
	c->BEnemy[4].LoadImage();

	c->p.x=100,c->p.y=50;
	c->Enemy[5].InitTank(3,3,8,1,2,6,c->p);
	c->Enemy[5].LoadImage();
	c->p.x+=25,c->p.y+=60;
	c->BEnemy[5].InitBullet(3,1,20,2,c->p);
	c->BEnemy[5].LoadImage();

	c->p.x=700,c->p.y=450;
	c->Enemy[6].InitTank(3,2,8,1,2,7,c->p);
	c->Enemy[6].LoadImage();
	c->p.x+=60,c->p.y+=25;
	c->BEnemy[6].InitBullet(2,1,20,2,c->p);
	c->BEnemy[6].LoadImage();

	c->p.x=700,c->p.y=650;
	c->Enemy[7].InitTank(3,1,8,1,2,8,c->p);
	c->Enemy[7].LoadImage();
	c->p.x+=25,c->p.y-=10;
	c->BEnemy[7].InitBullet(1,1,20,2,c->p);
...
BOOL Cmydia10::PreTranslateMessage(MSG* pMsg)
{
	bool Judge=false;
