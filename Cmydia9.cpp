...
IMPLEMENT_DYNAMIC(Cmydia9, CDialogEx)

Cmydia9::Cmydia9(CWnd* pParent /*=NULL*/)
	: CDialogEx(Cmydia9::IDD, pParent)
{
...
void Cmydia9::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(Cmydia9, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_QUERYDRAGICON()
END_MESSAGE_MAP()
...
BOOL Cmydia9::OnInitDialog()
{
	...
	CreateThread(NULL,0,InitTank2,this,0,NULL);
	CreateThread(NULL,0,MyPaint2,this,0,NULL);//启动线程*
	
	CreateThread(NULL,0,PlayerButtle2,this,0,NULL);
	CreateThread(NULL,0,PlayerMove2,this,0,NULL);
	CreateThread(NULL,0,EnemyMove02,this,0,NULL);
	CreateThread(NULL,0,EnemyMove12,this,0,NULL);
	CreateThread(NULL,0,EnemyMove22,this,0,NULL);
	CreateThread(NULL,0,EnemyMove32,this,0,NULL);
	CreateThread(NULL,0,EnemyMove42,this,0,NULL);
	CreateThread(NULL,0,EnemyMove52,this,0,NULL);
	CreateThread(NULL,0,EnemyMove62,this,0,NULL);
	CreateThread(NULL,0,EnemyMove72,this,0,NULL);
	

	return TRUE;  // 除非将焦点设置到控件，否则返回 TRUE
}

void Cmydia9::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR Cmydia9::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

DWORD WINAPI MyPaint2(LPVOID lpParam)			//贴图线程
{
	Cmydia9 *c=(Cmydia9*)lpParam;
	...
	c->Back.LoadBitmap(IDB_BITMAP17);//进入关卡的地图
...
DWORD WINAPI InitTank2(LPVOID lpParam)
{
	...
	c->p.x=1100,c->p.y=50;
	c->Enemy[0].InitTank(9,4,6,1,2,1,c->p);
	c->Enemy[0].LoadImage();
	c->p.x-=10,c->p.y+=25;
	...
	c->Enemy[1].InitTank(9,4,6,1,2,2,c->p);
	c->Enemy[1].LoadImage();
	c->p.x-=10,c->p.y+=25;
	...
	c->Enemy[2].InitTank(9,4,6,1,2,3,c->p);
	c->Enemy[2].LoadImage();
	c->p.x-=10,c->p.y+=25;
	...
	c->Enemy[3].InitTank(9,4,6,1,2,4,c->p);
	c->Enemy[3].LoadImage();
	c->p.x-=10,c->p.y+=25;
	...
	c->p.x=700,c->p.y=50;
	c->Enemy[4].InitTank(9,4,6,1,2,5,c->p);
	c->Enemy[4].LoadImage();
	c->p.x-=10,c->p.y+=25;
	...
	c->p.x=700,c->p.y=250;
	c->Enemy[5].InitTank(9,3,6,1,2,6,c->p);
	c->Enemy[5].LoadImage();
	c->p.x+=25,c->p.y+=60;
	...
	c->p.x=700,c->p.y=450;
	c->Enemy[6].InitTank(9,2,6,1,2,7,c->p);
	c->Enemy[6].LoadImage();
	c->p.x+=60,c->p.y+=25;
	...
	c->p.x=700,c->p.y=650;
	c->Enemy[7].InitTank(9,1,6,1,2,8,c->p);
	c->Enemy[7].LoadImage();
	c->p.x+=25,c->p.y-=10;
...
BOOL Cmydia9::PreTranslateMessage(MSG* pMsg)
{
	bool Judge=false;
